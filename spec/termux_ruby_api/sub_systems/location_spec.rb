...
EOT
  ...
  it "works without arguments" do
    expect(@base).to receive(:`).with('termux-location').once.and_return(result)
    @base.location.get
  end

  it "sends 'provider' argument (general and shorthands)" do
    expect(@base).to receive(:`).with('termux-location -p gps').twice.and_return(result)
    @base.location.get(provider: :gps)
    @base.location.gps
    expect(@base).to receive(:`).with('termux-location -p network').once.and_return(result)
    @base.location.network
  end

  it "sends 'request' argument" do
    expect(@base).to receive(:`).with('termux-location -r once').once.and_return(result)
    @base.location.get(request: :once)
  end

  it "sends both arguments" do
    expect(@base).to receive(:`).with('termux-location -p gps -r once').twice.and_return(result)
    @base.location.get(request: :once, provider: :gps)
    @base.location.gps(request: :once)
    expect(@base).to receive(:`).with('termux-location -p network -r once').once.and_return(result)
